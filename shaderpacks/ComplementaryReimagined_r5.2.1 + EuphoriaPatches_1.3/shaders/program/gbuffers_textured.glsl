/////////////////////////////////////
// Complementary Shaders by EminGT with Euphoria Patches by SpacEagle17 and isuewo //
/////////////////////////////////////
...
void main() {
        ...
        if (color.b > 1.15 * (color.r + color.g) && color.g > color.r * 1.25 && color.g < 0.425 && color.b > 0.75) { // Water Particle
            ...
            if (dotColor > 0.25 && color.g < 0.5 && (color.b > color.r * 1.1 && color.r > 0.3 || color.r > (color.g + color.b) * 3.0)) {
                ...
                lmCoordM = vec2(0.0);
                    ...
                    if (color.b > color.r * color.r && color.g < 0.16 && color.r > 0.2) color.rgb = normalize(netherColor) * 0.5; // Nether Portal
                #endif
                ...
                emission *= LAVA_EMISSION;
                    ...
                    color.rgb = fireColor(color.rgb, 3.5, colorSoul, inSoulValley);
                #endif
                #if defined PURPLE_END_FIRE && defined END
                    color.rgb = fireColor(color.rgb, 3.5, colorEndBreath, 1.0);
                #endif
            }
        }
    }
    ...
    #else
        #if defined OVERWORLD && defined NO_RAIN_ABOVE_CLOUDS || defined NETHER && (defined BIOME_COLORED_NETHER_PORTALS || defined SOUL_SAND_VALLEY_OVERHAUL)
                ...
                if (max(abs(colorP.r - colorP.b), abs(colorP.b - colorP.g)) < 0.001) {
                    if (dot(color.rgb, color.rgb) > 0.25 && color.g < 0.5 && (color.b > color.r * 1.1 && color.r > 0.3 || color.r > (color.g + color.b) * 3.0)) {
                            ...
                            if (color2.b > color2.r * color2.r && color2.g < 0.16 && color2.r > 0.2) {
                                ...
                                color.rgb = normalize(netherColor) * 0.5;
                                lmCoordM = vec2(0.0);
                            }
                        #endif
                    } else if (color.r > 0.83 && color.g > 0.23 && color.b < 0.4) {
