...
void main() {
    ...
    if (color.a > 0.00001) {
        ...
        #ifdef IS_IRIS
            if (currentRenderedItemId == 0 || currentRenderedItemId == 10028) {
                int heldBlockLight = (viewPos.x > 0.0 ^^ isRightHanded) ? heldBlockLightValue2 : heldBlockLightValue; // Separate in main and off-hand using x-or
                if (heldBlockLight > 0) {
                    noSmoothLighting = true;
                    emission = GetLuminance(color.rgb) * 2.5 * heldBlockLight / 15.0;
                }
            }
        #endif
