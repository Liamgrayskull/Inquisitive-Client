...
void main() {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (aspectRatio < 3) textCoord += vec2(3 * texCoordM.x * watermarkSize * 1.5 - 3 * watermarkSize * 1.5, 1.0 - 3 * a * texCoordM.y * watermarkSize * 1.5 / aspectRatio);
        ...
        if (textCoord.x > -1 && textCoord.x < 0 && textCoord.y > 0 && textCoord.y < 1) {
            ...
            #endif
            color.rgb = mix(color.rgb, EuphoriaPatchesText.rgb, EuphoriaPatchesText.a * guiIsNotHidden);
        }
    #endif
